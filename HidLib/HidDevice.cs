        ...
        private bool WriteData(byte[] data, int timeout)
        {
            ...
            else
            {
                try
                {
                    ...
                    var writeTask = Task.Factory.StartNew(state =>
                    {
                        var token = (CancellationToken)state;
                        while (!token.IsCancellationRequested)
                        {
                            writeStatus = NativeMethods.WriteFile(Handle, buffer, (uint)buffer.Length, out bytesWritten, ref overlapped);
                        }
                        token.ThrowIfCancellationRequested();
                    }, cts.Token, cts.Token);

                    if (writeTask.Wait(300, cts.Token))
                    {
                        cts.Cancel();
                        writeStatus = false;
                    }
            ...
            if (_deviceCapabilities.InputReportByteLength > 0)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        var cts = new CancellationTokenSource();
                        Task cancellable = readTask.ContinueWith(ignored => { }, cts.Token);

                        if (Task.WaitAny(new Task[] { readTask }, TimeSpan.FromMilliseconds(timeout)) == -1)
                        {
                            cts.Cancel();

                            Task.WaitAny(new[] { cancellable }, TimeSpan.FromSeconds(timeout));
                            NativeMethods.CancelSynchronousIo(Handle);
